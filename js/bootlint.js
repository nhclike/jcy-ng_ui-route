	...
	domManip: function( args, callback ) {
		...
		if ( l ) {
			...
			if ( first ) {
				...
				// Use the original fragment for the last case instead of the first because it can end up
...
function buildParams( prefix, obj, traditional, add ) {
	...
	if ( jQuery.isArray( obj ) ) {
		// Serialize array case.
		jQuery.each( obj, function( i, v ) {
			if ( traditional || rbracket.test( prefix ) ) {
				// Treat each array case as a scalar.
				add( prefix, v );
		...
		// Serialize object case.
		for ( name in obj ) {
			buildParams( prefix + "[" + name + "]", obj[ name ], traditional, add );
		}

	} else {
		// Serialize scalar case.
		add( prefix, obj );
	}
}
...
// Only the first case is strictly required.
...
(function (exports) {
    ...
    /**
     * Given a sorted array of integers, this finds all contiguous runs where each case is incremented by 1 from the next.
     * For example:
    ...
    addLinter("E041", function lintCarouselStructure($, reporter) {
        ...
        var innersWithWrongActiveItems = $('.carousel-inner').filter(function () {
            return $(this).children('.case.active').length !== 1;
        });
